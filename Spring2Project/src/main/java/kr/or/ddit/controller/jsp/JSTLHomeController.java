...
public class JSTLHomeController {
	 ...
	 * 		<c:choosse>		|	여러 조건을 처리할 때 사용한다. (when, otherwise)
	 ...
	 * 		<fn:containsIgnoreCase>	|	지정한문자열이 대문자/소문자를 구분하지 않고 포함되어 있는지 판단한다.
	 * 		<fn:startsWith>			|	지정한문자열로 시작하는지 판단한다.
	 * 		<fn:endsWith>			|	지정한문자열로 끝나는지 판단한다.
	 * 		<fn:indexOf>			|	지정한문자열이 처음으로 나왔을 때의 인덱스를 구한다.
     ...
     *		웹 애플리케이션 내에서의 절대 경로를 사용할 경우 실제로 새엉되는 URL은 컨텍스트 경로를 포함한다.
     *		
     *		12) <c:import>
     ...
     *			scope			|	String	|	변수를 저장할 영역
     *			charEncodeing	|	String	|	겨로가를 읽어올 때 사용할 캐릭터 인코딩
     *		────────────────────────────────────────────────────────────────────────
     *
     *		13) <c:redirect>
     *		- 지정한 URL에 리다이렉트한다.
     ...
     *		────────────────────────────────────────────
	 */
	// c:out excapeXml 속성의 기본값을 true이다.
	...
	// c:catch
	// EL안에서 발생한 에러가 아니라, 자바식에서 발생한 에러 정보이기 때문에 var 속성에 담긴 에러 정보를 확인할 수 있다.
