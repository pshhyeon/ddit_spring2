...
public class JSPFmtTagController {
	 ...
	 * 		기본값을 number이다. currency는 속해있는 국가코드에 맞는 통화 형식이 부여된다.
	 * 
	 * 		2) <fmt:parseNumber>
	 * 		- 문자열을 숫자로 변환한다.
	 * 		
	 ...
	 * 		dateStyle 속성 : default, short, medium, long, full 중 한가지 값을 가질 수 있고, 기본값은 default이다.
	 * 		timeStyle 속성 : default, short, medium, long, full 중 한가지 값을 가질 수 있고, 기본값은 default이다.
	 * 	
	 * 		4) <fmt:parseDate>
	 * 		- 문자열을 Date 객체로 변환한다.
	 * 	
	...
	// 7) type 속성을 time로 지정하여 시간 포맷팅을 한다.
