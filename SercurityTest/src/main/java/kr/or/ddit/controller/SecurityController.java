...
public class SecurityController {
	 ...
	 * 		타겟으로 접근하기 위한 요청을 날렸고 요청안에 사용자 등급에 해당하는 인가정보가 포함되어 있지 않으면 스프링 시큐리티는 
	 * 		인증을 진행할 수 있도록 인증 페이지(로그인 페이지)를 제공하여 사용자에게 인증을 요청합니다.
	 * 		사용자는 아이디, 비ㅁㄹ번호를 입력 후 인증을 요청하겠죠?
	 ...
	 * 		UserDetail 객체로 만들고, 이이디, 비밀번호, 권한들에 대한 정보들로 User 객체를 만듭니다.
	 * 		만들어진 객체 정보는 이후 SecurityContenxtHolder인 메모리 공간 내 존재하는 최종 객체 정보에서 꺼낼 값이됩니다.
	 ...
	 * 		전달해주면, AuthenticationFilter를 통해서 시큐리티 내 InMemory 공간(SecurityContextHolder)에 해단하는 영역안에 저장을 합니다.
	 * 		SecurityContextHolder안에 SecurityContext안에 Authentication 객체 안에 Principal 객체정보로
	 * 		User객체 안에 저장된 정보를 담아둡니다.
	 * 		그리고 최종 타겟으로 넘어가기 전 JSESSIIONID인 세션에 대한 쿠키정보값이 유효한지를 체킹 후, 유효하다면 타겟정보로
	 ...
	 * 			- 스프링 시큐리티가 제공하는 서블릿 필터 클래스 서블릿 컨테이너에 등록한다. (web.xml)\
	 * 			- 스프링 시큐리티는 필터 기반으로 시큐리티 필터 체인을 등록한다.
	 ...
	 * 
	 * 	3. 접근 제한 설정
	 * 	- 시큐리티 설정을 통해서 특정 URL에 대한 접근을 제한 할 수 있다.
	 * 
	 * 		# 환경 설정
	 * 		- 스프링 시큐리티 설정
	 ...
	 * 				> 접근 제한에 결려 스프링 시큐리티가 기본적으로 제공하는 로그인 페이지로 이동합니다.
	 ...
	 * 	- 암호화 처리기를 사용하지 않도록 하기 위해서는 "{noop}" 문자열을 비밀번호 앞에 사용한다.
	 * 
	 * 		# 환경 설정
	 * 		- 스프링 시큐리티 설정
	 * 			> security-context.xml 설정
	 * 		- <security:quthentication-manager>
	 * 		-	<security:quthentication-provider>
	 ...
	 * 			> 관리자 등급인 admin 계쩡은 ROLE_MEMBER 권한도 가지고 있는 계정이므로 해당 페이지로 접근 가능
	 * 		- 공지사항 게시판 등록 화면
	 * 			> 접근 제한에 걸려 스프링 시큐리티가 기본적으로 제공하는 로그인 페이지가 연결되고,
	 ...
	 * 				공지사항 게시판 등록 페이지는 관리자 권한으로만 접근 가능하믈 접근이 거부된다.
	 ...
	 * 			> 접근 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고 회원 권한을 가진 계정으로 접근 시 접근 가능
	 * 		- 공지사항 게시판 등록 페이지
	 * 			> 접근 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고ㅡ 회원 권한을 가진 계정으로 접근 시에
	 * 				공지사항 게시판 등록 페이지는 관리자 권한으로 접근 간으하므로 접근이 거부된다.
	 * 				이때, acess-denied-handler로 설정되어 있는 ref 속성에 부여된 클래스 메소드로 이동하고 해당 페이지에서
	 ...
	 * 				: 사용자가 직접 만든 로그인 페이지로 이동알 '/login' URI를 가지고 있는 컨트롤러 메소드를 정의합니다.
	 * 
	 * 
	 * 		# 로그인 페이지 정의 
	 * 		- 사용자가 정의한 로그인 컨트롤러
	 ...
	 * 		그렇지 않은 경우 사용자가 만들어 놓은 접근 거부 페이지로 이동합니다.
	 * 
	 * 8. 로그인 성공 처리
	 ...
	 * 		# 로그인 성공 처리자 클래스 정의
	 * 			- 로그인 성공 처리자
	 * 			> SaveRequestAwareAuthenticationSuccesHandler는 AuthenticationSuccessHandler의
	 * 				구현 클래스이다. 인증 전에 접근을 시도한 URL로 리다이렉트하는 기능을 가지고 있으며 스프링 시큐리티에서
	 * 				기본적으로 사용되는 구현 클래스입니다.
	 * 			- 로그인 성공 처리자 2
	 * 			> AuthenticationSuccessHandler 인터페이스를 직접 구현하여 인증 전에 접근을 시도한 URL로 리다이렉트하는 기느을 구현한다.
	 * 
	 * 		# 화면 설명
	 * 			- 일반 게시판 등록 화면
	 * 				> 사용자가 정의한 록그인 페이지에서 회원권한에 해당하는 계정으로 로그인 시, 성공했다면 성공 처리자인
	 ...
	 * 
	 * 			** logout 경로는 스프링에서 저공하는 /logout경로로 설정한다.
	 * 
	 * 			** 스프링 시큐리티 로그아웃에서 제공되는 logout-url속성은 로그아웃을 요청할 경로,
	 * 				기본값은 '/logout' 경로로 설정되어있다.
	 * 				원하는 경로를 지정할 수 있으며, 기본적으로 method방식은 POST로 동작한다.
	 * 				로그아웃이 완료된 이우헤 /login?logout이라는 경로로 자동 넘어간다. (별도의 커스텀 가능)
	 * 
	 ...
	 * 				> <sercurity:authentication-manager> 태그 설정
	 * 
	 * 		# 비밀번호 암호화 처리기 클래스 정의
	 * 			- 비밀번호 암호화 처리기
	 ...
	 * 
	 * 			- 권한을 확인할 때 필요한 쿼리
	 * 			> select m.user_id, auth from member_auth ma, member m where ma.user_no and m.user_id = ?
	 * 
	 * 			-BcryptPasswordEncoder클래스를 활용한 단방향 비밀번호 암호화
	 * 			> encode() 메소드를 톻해서 SHA-2방식의 8바이트 hash 암호화를 매번 랜덤하게 생성합니다.
	 ...
	 * 			수십억개의 다이제스트들 중에서 일치하는 다이제스트가 존재할 경우 비밀번호의 일치로 보고 인증을 성공시켜준다.
	 * 
	 ...
	 *  			> 데이터베이스 관련 라이브러리
	 *  
	 *  			- 스프링 시큐리티 설정(sercurity-context.xml 설정)
	 ...
	 * 
	 * 14. 자동 로그인
	 * - 로그인하면 특정 시간동안 다시 로그인 할 필요가 없는 기능이다.
	 ...
	 * 					token-calidity-seconds="604800"/> 태그 설정
	 * 				> security:logout log-url="/logout" invalidate-session="true"
	 ...
	 * 			- JSESSIONID를 삭제 후, 다시 로그인을 진행하더라도 로그인 후 진행될 페이지가 정상적으로 나타나는걸 확인할 수있다.
	 ...
	 * 		# 사용 어노테이션
	 * 			- @Secured
	 * 				> 스프링 시큐리티 모듈을 지원하기 위한 어놑이션으로 초기부터 사용되었다.
	 *			- @PreAuthorize
